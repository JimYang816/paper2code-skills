...
minimum Evidence Strength. The contract must be present before Full Run
approval so the gate can bind its hash. Evaluation reads this contract; it never edits
...
cannot receive `high` Evidence Strength. Evidence Strength also accounts for
validation coverage and observed variability. A decisive result mismatch is a
claim outcome routed to `revision_required`, not permission to tune the
implementation or tolerance.
